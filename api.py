...
def atualizar_produto(produto_id):
    ...
    try:
        ...
        for campo in campos_permitidos:
            if campo in dados:
                if campo in ['preco', 'preco_original', 'score_tendencia']:
            ...
            function carregarProdutos() {
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        if (!data.produtos) {
                            div.innerHTML = '<p>Erro ao carregar produtos.</p>';
                            return;
                        }
                        
                        let html = '<h3>📋 Produtos Cadastrados:</h3><table border="1" cellpadding="8" cellspacing="0" style="width:100%; border-collapse: collapse;">';
                        html += '<tr style="background:#4CAF50; color:white;"><th>ID</th><th>Nome</th><th>Categoria</th><th>Preço</th><th>Plataforma</th><th>Score</th><th>Ações</th></tr>';
                        ...
                        data.produtos.forEach(p => {
                            ...
                            html += '<td>R$ ' + p.preco.toFixed(2) + '</td>';
                            html += '<td>' + p.plataforma + '</td>';
                            html += '<td>' + p.score_tendencia + '</td>';
                            html += '<td><button onclick="removerProduto(' + p.id + ')">Remover</button> <button onclick="editarProduto(' + p.id + ')">Editar</button></td>';
                            html += '</tr>';
                        });
                ...
                fetch('/api/produtos/estatisticas')
                    ...
                    .then(data => {
                        ...
                        if (data.estatisticas && data.estatisticas.total_produtos > 0) {
                            statsDiv.innerHTML = '<div class="filtros"><h3>📊 Estatísticas</h3>' +
                                ...
                                '<p>Categorias: ' + data.estatisticas.categorias.join(', ') + '</p></div>';
                        }
                    })
                    .catch(error => console.log('Estatísticas:', error));
            }
